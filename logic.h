...
void updateObstacles(){
    ...
    for (auto& obs : obstacles) {
        if (car.x > obs.x && !obs.passed) {
            score += 1; // Tăng điểm
            obs.passed = true; // Đánh dấu đã vượt qua
        }
    }
...
void updateFuelCans() {
    ...
    fuelCans.erase(std::remove_if(fuelCans.begin(), fuelCans.end(),
                                  [](const FuelCan& fuelCan) { return fuelCan.x < car.x - SCREEN_WIDTH; }),
                   fuelCans.end());
}
...
void checkFuelPickup() {
    ...
    for (auto& fuelCan : fuelCans ) {
        if (fabs(car.x - fuelCan.x) < 20){
            car.fuel = std::min(car.fuel + 5, 100.0f);
            fuelCan.x = -100;
        }
    }
}
...
void resetGame(bool &gameover) {
    ...
    if (!terrain.points.empty()) {
        car.y = terrain.points[0].y - 20; // Đặt xe trên điểm đầu tiên
    } else {
        car.y = 300 - 20; // Giá trị mặc định nếu địa hình rỗng
    }
}
...
void checkObstacleCollision(bool& running) {
    for (auto& obs : obstacles) {
        SDL_Rect carRect = {(int)car.x - 20, (int)car.y - 10, 40, 20}; // Kích thước xe
