...
bool loadMedia() {
    ...
    engineSound = Mix_LoadWAV ("engine.wav");
    ...
    return car.carTexture && car.wheelTexture  && fuelCan.fuelCanTexture && backgroundMusic && terrainTexture && grassTexture && engineSound
    && obstacle.obstacleTexture && gameOverTexture && font;
}
void render() {
    ...
    SDL_RenderClear(renderer);
     ...
     for (size_t i = 0; i < terrain.points.size() - 1; ++i) {
            ...
            // Tính độ dài đoạn địa hình
            float segmentLength = sqrt(dx * dx + dy * dy);
            // Đặt cỏ với khoảng cách nhỏ để trông tự nhiên
            int step = grassWidth / 2;  // Cỏ cách nhau gần hơn
        ...
        if (terrain.points.size() >= 2) {
            for (size_t i = 0; i < terrain.points.size() - 1; ++i) {
                if (obs.x >= terrain.points[i].x && obs.x <= terrain.points[i + 1].x) {
                    renderY = terrain.points[i].y - obs.height; // Đặt trên địa hình
                    break;
                }
            }
        }
    ...
    if (gameover) {
        ...
        // Vẽ nút Restart
        SDL_Rect restartButton = {(SCREEN_WIDTH - 100) / 2, SCREEN_HEIGHT / 2 + 20, 100, 40};
        SDL_SetRenderDrawColor(renderer, 100, 100, 100, 255); // Màu xám cho nút
...
int main(int argc, char* argv[]) {
    ...
    createFuelCans();
    createObstacles();
    car.x = 100; car.y = terrain.points[0].y - 20; car.velocityX = car.velocityY = 0;
    bool running = true;
